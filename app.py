...
# =======================================================
# 3. LOAD AI MODELS (Warfarin & Diabetes) & SYSTEM CHECKS
# =======================================================

@app.before_request
def check_maintenance_and_db():
    # SUPABASE SAFE INIT: Add missing tables, don't drop existing ones
    try:
        inspector = inspect(db.engine)
        if not inspector.has_table("user"):
             with app.app_context():
                db.create_all()
        elif not inspector.has_table("admin_email"):
             with app.app_context():
                db.create_all()
    ...
    # Enterprise Maintenance Lockout Check
    basedir = os.path.abspath(os.path.dirname(__file__))
